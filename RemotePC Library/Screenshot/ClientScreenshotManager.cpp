...
int CClientScreenshotManager::Decompress(BYTE *pCompressedBuffer, DWORD CompressedBufferSize)
{
	...
	int Res = 0;
	{
		...
		// Decompress the data
		Res = FreeImage_ZLibUncompress(pOut, UncSize, pIn, CompressedDataSize);
		if(Res == 0)
			return Res;
	}
